		...
		var button = $('<'+buttonDom.tag+'/>')
			...
			.on( 'click.dtb', function (e) {
				if (!config.exportURL) {
					...
					//set the visiblity variable 
					for (var i = 0; i < dt.ajax.params().columns.length; i++) {
						dt.ajax.params().columns[i].visible = dt.column(i).visible()
					}
					// download the csv from given url
            		window.location.href = config.exportURL + '?'+$.param(dt.ajax.params())
				}
				button.blur();
			} )
 ...
 */
Buttons.defaults = {
	buttons: [ 'copy', 'excel', 'csv', 'pdf', 'print', 'daterangepicker' ],
	name: 'main',
	tabIndex: 0,
	dom: {
		container: {
...
$.extend( _dtButtons, {
	...
	daterangepicker: function ( dt, conf ) {
		// Common option that will use the HTML5 or Flash export buttons
		if ( _dtButtons.daterangeHtml5 && _dtButtons.daterangeHtml5.available( dt, conf ) ) {
			return 'daterangeHtml5';
		}
		if ( _dtButtons.daterangeFlash && _dtButtons.daterangeFlash.available( dt, conf ) ) {
			return 'daterangeFlash';
		}
	}
} );
